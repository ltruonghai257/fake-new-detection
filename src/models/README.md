...
Note APP:
/opsx-explore Please go from crawled data from /Users/haila/Library/CloudStorage/GoogleDrive-latruonghai@gmail.com/My Drive/Thesis_Final/fake-new-detection/notebooks/data/json, and with COOLANT model, is my data align with model architect. Find out why my training do not improve? Go from data crawled -> preprocessing text and image -> training. Give me the root cause or potential issue make training is underfitting?

/opsx-propose I want to. traing 3 module individually before training all (confirm with paper please). My preprocessing may match with CLIP training, for all, may be generate the label again. Please check in ViFactCheck getting data and want to have their label for trainnig
