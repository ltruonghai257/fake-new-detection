...
class SimilarityModule(nn.Module):
    ...
    def __init__(
        self, text_input_dim: int = 200, shared_dim: int = 128, sim_dim: int = 64
    ):
        super(SimilarityModule, self).__init__()

        self.encoding = EncodingPart(text_input_dim=text_input_dim)
...
class DetectionModule(nn.Module):
    ...
    def __init__(
        self, text_input_dim: int = 200, feature_dim: int = 96, h_dim: int = 64
    ):  # 64 + 16 + 16
...
class COOLANT(MultimodalModel):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Get text input dimension from config
    ...
    def encode_text(self, text: torch.Tensor) -> torch.Tensor:
        """Encode text input."""
        text_shared, _ = self.similarity_module.encoding(
    ...
    def encode_image(self, image: torch.Tensor) -> torch.Tensor:
        """Encode image input."""
        _, image_shared = self.similarity_module.encoding(
    ...
    def forward(
        ...
        if text_aligned is None or image_aligned is None:
            text_aligned, image_aligned, similarity_pred = self.similarity_module(
                text_raw, image_raw
            )
        else:
    ...
    def compute_total_loss(
        ...
        contrastive_loss = self.compute_contrastive_loss(
            outputs["text_features"], outputs["image_features"]
        )
        ...
        if similarity_labels is not None and "similarity_pred" in outputs:
            similarity_loss = F.cross_entropy(
                outputs["similarity_pred"], similarity_labels
            )
