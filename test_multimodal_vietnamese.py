...
def test_multimodal_preprocessing():
    ...
    print(
        f"✓ Labels distribution: Real: {sum(1 for l in labels if l == 0)}, Fake: {sum(1 for l in labels if l == 1)}"
    )
    ...
    # Use the existing split files in src/data/json
    data_dir = "./src/data/json"

    # Process the existing splits
    print(f"🔍 Looking for split files in: {os.path.abspath(data_dir)}")
    datasets = combined_preprocessor.process_existing_splits(
        data_dir=data_dir,
...
def test_data_loading():
    ...
    if os.path.exists(data_path):
        text_features, image_features, labels = (
            CombinedPreprocessor.load_combined_dataset(data_path)
        )
        ...
        print(
            f"✓ Sample data shapes: Text={sample_text.shape}, Image={sample_image.shape}, Label={sample_label.shape}"
        )
