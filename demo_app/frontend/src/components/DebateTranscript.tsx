import { DebateTurn, WeightBreakdown } from '../types';
...
function VerdictBadge({
    ...
    verdict: 'REAL' | 'FAKE' | null | undefined;
}) {
...
function TurnBubble({
    ...
    return (
        <div
            className={`flex ${isReal ? 'justify-start' : 'justify-end'} mb-3`}>
            <div
                className={[
                ...
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-xs font-semibold text-gray-500">
                        {isReal ? 'Bảo vệ' : 'Phản biện'} · Vòng{' '}
                        {turn.round + 1}
                    </span>
                    <VerdictBadge
                        verdict={turn.verdict as 'REAL' | 'FAKE' | null}
                    />
    ...
    return (
        <div className="bg-white rounded-xl shadow p-4">
            ...
            {currentTurnAgent && (
                <div
                    className={`flex ${
                        isCurrentReal ? 'justify-start' : 'justify-end'
                    } mb-3`}>
                    <div
                        className={[
                            'max-w-[75%] rounded-xl p-4',
                        ...
                        <div className="text-xs font-semibold text-gray-500 mb-1">
                            {isCurrentReal ? 'Bảo vệ' : 'Phản biện'} · Đang
                            viết...
                        </div>
                        <p className="text-gray-800 text-sm whitespace-pre-wrap">
                            {currentTurnText}
            ...
            {debateConverged && debateAgreedVerdict && (
                <div
                    ...
                    Hai bên đồng thuận:{' '}
                    <strong>
                        {debateAgreedVerdict === 'REAL' ? 'THẬT' : 'GIẢ'}
                    </strong>
                </div>
            )}
        </div>
