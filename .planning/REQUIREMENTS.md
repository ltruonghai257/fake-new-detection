...
-   [ ] **GRAPH-04**: LangGraph checkpointer (`SqliteSaver`, falling back to `MemorySaver` if the sqlite extra is unavailable) is added via `g.compile(checkpointer=...)`; callers pass a `thread_id` in the invoke config; an interrupted run resumes from the last completed node instead of restarting from search
...
_Last updated: 2026-07-19 after initial definition_
